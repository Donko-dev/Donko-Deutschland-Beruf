...
## 📸 Galerie dynamique (zéro code)

La section **Galerie** de la page se met à jour **automatiquement pour tout le monde**, sans toucher au code, dès que vous déposez un fichier sur GitHub :

1. Créez (ou ouvrez) un dossier `gallery/` à la racine du dépôt.
2. Glissez-déposez une image (`.png .jpg .jpeg .gif .webp .svg .avif`) ou une vidéo (`.mp4 .webm .mov .ogg`) directement dans ce dossier via l'interface GitHub ("Add file" → "Upload files").
3. Validez le commit (*"Commit changes"*).
4. Rechargez le site : le fichier apparaît dans la galerie de tous les visiteurs.

Pour retirer un média, supprimez-le simplement du dossier `gallery/` sur GitHub — il disparaît du site automatiquement.

**Comment ça marche techniquement :** la page interroge en direct l'API publique de GitHub (`api.github.com/repos/.../contents/gallery`) pour lister le contenu du dossier, sans base de données ni backend. ⚠️ Cette API est limitée à 60 requêtes/heure par visiteur non connecté — largement suffisant pour un site vitrine, mais à garder en tête en cas de pic de trafic inhabituel. Si la limite est atteinte ou en cas de coupure réseau, un message d'indisponibilité s'affiche proprement au lieu d'une erreur.
